...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupItem() {
        // 画像のスプライトを作成
        let itemSprite = SKSpriteNode(imageNamed: "item.png")
        
        // 配置位置を決める
        itemSprite.position = CGPoint(x:self.frame.midX, y:self.frame.midY)
        
        // サイズを変更
        itemSprite.size = CGSize(width: 10, height: 10)
    ...
    func setupScoreLabel() {
        ...
        itemScoreLabelNode.position = CGPoint(x: 235, y: self.frame.size.height - 30)
